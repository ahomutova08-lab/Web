...
<main class="container">
  ...
  <!-- ── Banner rezultate PHP (când e activ un filtru) ── -->
  <?php if ($searchQ !== '' || $filterCat !== 'all'): ?>
    <div class="results-banner" role="status" aria-live="polite">
      <?php if ($totalVisible > 0): ?>
        Am găsit <strong><?= $totalVisible ?></strong>
        <?= $totalVisible === 1 ? 'rețetă' : 'rețete' ?>
        <?php if ($searchQ !== ''): ?>
          pentru „<em><?= e($searchQ) ?></em>"
        <?php endif; ?>
        <?php if ($filterCat !== 'all'): ?>
          în categoria <strong><?= e($filterCat) ?></strong>
        <?php endif; ?>
      <?php else: ?>
  ...
  <!-- ── Lista de rețete grupată pe categorii ── -->
